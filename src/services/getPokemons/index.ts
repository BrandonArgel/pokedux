...
export const getPokemonsService = async (page: number) => {
  const { count, next, results, previous } = await fetch(
    `${URL_API_BASE}pokemon/?limit=20&offset=${(page - 1) * 20}`
  )
  ...
  const pokemons = await Promise.all(
    results.map(async (pokemon: PokemonBaseModel) => {
      const { id, sprites, types } = await fetch(pokemon.url).then((response) =>
        response.json()
      );
